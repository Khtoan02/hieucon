    ...
    <div id="page" class="site">
        <header id="main-header" class="glass-header sticky top-0 z-[100] w-full" aria-label="Main Navigation">
            <div class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
                <!-- Header Container -->
                <div
                    class="header-container flex justify-between items-center h-[60px] lg:h-[72px] transition-all duration-400">

                    <!-- 1. Logo & Brand -->
                    <a href="<?php echo home_url('/'); ?>"
                        class="flex items-center gap-3 cursor-pointer group shrink-0 outline-none focus-visible:ring-2 focus-visible:ring-secondary rounded-xl p-1">
                        <div class="transition-all duration-500 group-hover:scale-105 flex items-center justify-center">
                            <?php if (has_site_icon()): ?>
                                <img src="<?php echo esc_url(get_site_icon_url(96)); ?>" alt="<?php bloginfo('name'); ?>"
                                    class="w-10 h-10 lg:w-12 lg:h-12 rounded-xl object-cover shadow-sm bg-white">
                            <?php else: ?>
                                <div
                                    class="w-10 h-10 lg:w-12 lg:h-12 bg-navy text-white flex items-center justify-center rounded-xl shadow-sm">
                                    <i data-lucide="dna" class="w-6 h-6" aria-hidden="true"></i></div>
                            <?php endif; ?>
                        </div>
                        <div class="flex flex-col justify-center">
                            <h1
                                class="font-serif font-bold text-lg lg:text-[22px] leading-none text-navy tracking-tight">
                                HIỂU CON TỪ GỐC</h1>
                            <p
                                class="text-[9px] lg:text-[10px] font-extrabold text-navy/50 tracking-[0.1em] lg:tracking-[0.2em] uppercase mt-1 lg:mt-1.5 leading-none">
                                Tự kỷ là rối loạn toàn thân</p>
                        </div>
                    </a>

                    <!-- 2. Main Navigation (Desktop) -->
                    <nav
                        class="hidden lg:flex items-center h-full absolute left-1/2 -translate-x-1/2 space-x-6 xl:space-x-8 2xl:space-x-10">

                        <!-- Mega Menu "Sản phẩm" -->
                        <div class="group h-full flex items-center">
                            <button
                                class="text-navy/80 hover:text-navy group-hover:text-secondary font-extrabold transition-colors text-[12px] xl:text-[13px] uppercase tracking-[0.15em] flex items-center gap-1.5 py-4 relative outline-none px-2"
                                ...
                                <i data-lucide="chevron-down"
                                    ...
                                    aria-hidden="true"></i>
                                <span
                                    class="absolute bottom-0 left-1/2 -translate-x-1/2 w-0 h-[3px] bg-secondary transition-all duration-300 group-hover:w-full opacity-0 group-hover:opacity-100 rounded-t-md"></span>
                            </button>

                            <!-- Dropdown Panel -->
                            <div
                                class="absolute top-full left-1/2 -translate-x-1/2 pt-4 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform translate-y-3 group-hover:translate-y-0 w-[850px] xl:w-[980px] z-50">
                                <div
                                    class="mega-bridge glass-megamenu rounded-[2rem] shadow-premium p-6 xl:p-8 flex gap-4 xl:gap-6 relative overflow-hidden">

                                    <i data-lucide="sparkles"
                                        class="absolute -bottom-6 -right-6 w-32 h-32 text-navy/5 rotate-12 pointer-events-none"
                                        aria-hidden="true"></i>

                                    <!-- Cột 1: Thông tin & Trích dẫn -->
                                    <div class="w-[30%] border-r border-navy/10 pr-6 flex flex-col z-10 relative">
                                        <h4
                                            class="font-serif font-bold text-navy text-xl xl:text-2xl mb-4 leading-tight">
                                            Y Sinh Cốt Lõi</h4>

                                        <div
                                            class="p-6 bg-gradient-to-br from-secondary/5 to-[#FFF9F0] rounded-2xl border border-secondary/10 relative overflow-hidden mb-6 flex-grow flex flex-col justify-center">
                                            <i data-lucide="quote"
                                                class="absolute right-3 top-3 w-16 h-16 text-secondary/5 rotate-12"
                                                aria-hidden="true"></i>
                                            <p
                                                class="text-[14px] xl:text-[15px] text-navy/80 font-medium italic relative z-10 leading-relaxed font-serif">
                                                "Sức khỏe bền vững bắt nguồn từ việc thiết lập lại sự cân bằng toàn
                                                thân, từ gốc rễ tế bào."
                                            </p>
                                        </div>

                                        <a href="/san-pham"
                                            class="group/btn flex items-center justify-center gap-3 text-white bg-navy hover:bg-secondary font-bold text-[14px] p-4 rounded-xl shadow-md hover:shadow-lg transition-all duration-300 w-full uppercase tracking-wider">
                                    ...
                                    <div class="w-[70%] pl-6 flex flex-col z-10 relative overflow-hidden">
                                        <div class="flex justify-between items-center mb-4 pr-1">
                                            <span
                                                class="text-secondary font-extrabold text-[11px] uppercase tracking-widest flex items-center gap-2">
                                                <i data-lucide="star" class="w-4 h-4 fill-secondary/20"
                                                    aria-hidden="true"></i> Sản phẩm nổi bật
                                            </span>
                                            <div class="flex gap-2">
                                                <button id="mega-prev"
                                                    class="w-8 h-8 flex items-center justify-center rounded-full bg-white border border-navy/10 hover:border-secondary/40 hover:text-secondary text-navy/40 transition-colors shadow-sm outline-none"
                                                    aria-label="Lùi lại"><i data-lucide="chevron-left"
                                                        class="w-5 h-5"></i></button>
                                                <button id="mega-next"
                                                    class="w-8 h-8 flex items-center justify-center rounded-full bg-white border border-navy/10 hover:border-secondary/40 hover:text-secondary text-navy/40 transition-colors shadow-sm outline-none"
                                                    aria-label="Tiếp theo"><i data-lucide="chevron-right"
                                                        class="w-5 h-5"></i></button>
                                            </div>
                                        </div>
                                        ...
                                        <!-- Slider -->
                                        <div id="mega-slider"
                                            class="flex gap-5 overflow-x-auto snap-x snap-mandatory scroll-smooth pb-4 no-scrollbar">
                                            <?php
                                            ...
                                            foreach ($mega_products_data as $product):
                                                ...
                                                <a href="/<?php echo esc_attr($product['slug']); ?>"
                                                    class="shrink-0 w-[240px] snap-start bg-gradient-to-br from-white to-[#f8fafc] rounded-[1.5rem] p-4 border border-white hover:border-secondary/30 hover:shadow-elegant transition-all duration-300 group/card relative overflow-hidden flex flex-col">
                                                    <div
                                                        class="w-full h-[140px] bg-white rounded-xl mb-4 overflow-hidden shadow-sm flex items-center justify-center p-2 relative">
                                                    ...
                                                    <h5
                                                        class="font-serif font-bold text-lg text-navy mb-1.5 group-hover/card:text-secondary transition-colors leading-tight line-clamp-1">
                                                        <?php echo esc_html($product['title']); ?></h5>
                                                    <p
                                                        class="text-[12px] text-navy/60 leading-relaxed mb-4 flex-grow line-clamp-2">
                                                        <?php echo esc_html($product['desc']); ?></p>
                                                    <span
                                                        class="text-[11px] font-bold text-navy flex items-center gap-1 group-hover/card:text-secondary uppercase tracking-wider mt-auto">
                                                        Xem chi tiết <i data-lucide="arrow-right"
                                                            class="w-3 h-3 group-hover/card:translate-x-1 transition-transform"></i>
                                                    </span>
                                                </a>
                                            <?php endforeach; ?>
                                        </div>
                        ...
                        <a href="<?php echo esc_url(get_post_type_archive_link('ebook')); ?>"
                            class="text-navy/80 hover:text-navy hover:text-secondary font-extrabold transition-colors text-[12px] xl:text-[13px] uppercase tracking-[0.15em] flex items-center gap-1.5 py-4 relative outline-none px-2 shrink-0">
                            Thư viện tài liệu
                            <span
                                class="absolute bottom-0 left-1/2 -translate-x-1/2 w-0 h-[3px] bg-secondary transition-all duration-300 hover:w-full opacity-0 hover:opacity-100 rounded-t-md"></span>
                        </a>
                    ...
                    <div class="hidden lg:flex flex-1 justify-end items-center min-w-0 gap-3">
                        ...
                        if ($current_member):
                            ...
                            <a href="<?php echo home_url('/tai-khoan/'); ?>"
                                ...
                                <i data-lucide="user"
                                    class="w-4 h-4 text-secondary group-hover:text-white transition-colors"></i>
                                <span>Tài khoản</span>
                            </a>
                        <?php else: ?>
                    ...
                    <div class="lg:hidden flex items-center">
                        <button id="mobile-menu-btn" aria-label="Mở menu điều hướng" aria-expanded="false"
                            class="text-navy p-2.5 bg-white/70 rounded-xl border border-white shadow-sm hover:bg-white transition-colors">
