...
// Mock Data Generator for DH
if (!function_exists('hieucon_dh_generate_mock_data')) {
    function hieucon_dh_generate_mock_data() {
        global $wpdb;
        $table_name = $wpdb->prefix . 'hieucon_dh_checklists';
        ...
        for ($i = 0; $i < 50; $i++) {
            ...
            $da = [
                ...
                'referrer' => $ref,
                'toggles' => ['tieuHoa' => rand(0, 3), 'vanDong' => rand(0, 3)],
                'thinkTimes' => ['tieuHoa' => rand(5, 30), 'vanDong' => rand(5, 30), 'anUong' => rand(5, 30)],
...
if (isset($_GET['generate_mock']) && $is_local_env) {
    hieucon_dh_generate_mock_data();
    wp_redirect(remove_query_arg('generate_mock'));
    exit;
}

// Define DH helper functions
if (!function_exists('hieucon_dh_parse_device_details')) {
    function hieucon_dh_parse_device_details($ua) {
        if (empty($ua)) {
...
if (!function_exists('hieucon_dh_get_hesitation_details')) {
    function hieucon_dh_get_hesitation_details($da, $known_groups) {
        $toggles = $da['toggles'] ?? [];
...
if (!function_exists('hieucon_dh_get_traffic_source')) {
    function hieucon_dh_get_traffic_source($da) {
...
if (!function_exists('hieucon_dh_get_lead_quality')) {
    function hieucon_dh_get_lead_quality($r, $da) {
...
if (!function_exists('hieucon_dh_calculate_5_layer_insights')) {
    function hieucon_dh_calculate_5_layer_insights($rows, $known_groups, $group_items) {
        $completed_rows = [];
        foreach ($rows as $r) {
...
$group_items = [
    ...
    'anUong' => [
        'Trẻ chỉ chấp nhận một số rất ít món ăn quen thuận?',
        'Trẻ rất nhạy với mùi, màu, vị hoặc kết cấu thức ăn?',
        'Trẻ hay ngậm lâu, không nhai, nhai nuốt kém, buồn nôn hoặc oẹ khi gặp món lạ?',
        'Chế độ ăn kém đa dạng khiến con chậm tăng cân, sụt cân, mệt mỏi hoặc có các dấu hiệu thiếu vi chất?',
        'Trẻ ăn/nhai vật không phải thức ăn hoặc bùng nổ dữ dội khi bị ép ăn?'
...
foreach ($rows as $r) {
    ...
    $dev = hieucon_dh_parse_device_details($r->device_info);
    ...
    $src = hieucon_dh_get_traffic_source($da);
    $stat_sources[$src] = ($stat_sources[$src] ?? 0) + 1;
    
    $lq = hieucon_dh_get_lead_quality($r, $da);
    $stat_leads[$lq['level']]++;
...
$insights = hieucon_dh_calculate_5_layer_insights($rows, $known_groups, $group_items);
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Dashboard Phân Tích & Đo Lường (DH) - Hiểu Con Từ Gốc</title>
...
<body class="min-h-screen text-slate-800 transition-colors duration-200">
    ...
    <aside class="fixed left-0 top-0 z-40 h-screen w-[290px] border-r border-solid border-slate-200 bg-white transition-transform lg:translate-x-0 -translate-x-full" id="sidebar-drawer">
        <div class="flex items-center gap-3 px-6 py-6 border-b border-solid border-slate-100">
            ...
            <div>
                ...
                <p class="text-[9px] text-brand-600 font-bold tracking-widest uppercase mt-0.5">DOCUMENTING HOPE (DH)</p>
            </div>
        </div>
                            ...
                            <tbody class="divide-y divide-slate-100 text-sm">
                                <?php foreach ($group_prevalence as $gid => $gdata): ?>
                                    <tr class="hover:bg-slate-50 transition-colors cursor-pointer" onclick="togglePrevalenceDetail('<?php echo esc_js($gid); ?>')">
                                        <td class="py-3 font-semibold text-slate-800 pl-2">
            ...
            <div id="tab-prevalence" class="tab-content space-y-6 hidden">
                ...
                <div id="subtab-content-analysis" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        <!-- Card 1: Micro PSS -->
                        <div class="bg-white p-5 rounded-2xl border border-solid border-slate-200 shadow-sm relative group">
                            <div class="flex items-center justify-between">
                                <span class="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1">Micro_PSS Trung Bình 📌</span>
                            ...
                            <div class="tooltip top-1 right-2 bg-[#1e293b] text-white p-2.5 rounded-lg shadow-xl text-xs max-w-[240px] pointer-events-none absolute hidden group-hover:block z-50">
                        ...
                        <div class="bg-white p-5 rounded-2xl border border-solid border-slate-200 shadow-sm relative group">
                            <div class="flex items-center justify-between">
                                <span class="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1">Overload Index (SEOI) 📌</span>
                        ...
                        <div class="bg-white p-5 rounded-2xl border border-solid border-slate-200 shadow-sm relative group">
                            <div class="flex items-center justify-between">
                                <span class="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1">Regression (RRI) 📌</span>
                        ...
                        <div class="bg-white p-5 rounded-2xl border border-solid border-slate-200 shadow-sm relative group">
                            <div class="flex items-center justify-between">
                                <span class="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1">Tiến Triển Δ 📌</span>
                    ...
                    <div class="bg-white p-6 rounded-2xl border border-solid border-slate-200 shadow-sm relative">
                        ...
                        <div class="space-y-3">
                            ...
                            $symptom_short_desc = [
                                ...
                                'Trẻ chỉ chấp nhận một số rất ít món ăn quen thuận?' => 'Kén ăn món quen thuộc',
                            ...
                            <tbody class="divide-y divide-slate-100 text-sm">
                                ...
                                <?php else: ?>
                                    <?php foreach ($rows as $row): 
                                        ...
                                        $lq = hieucon_dh_get_lead_quality($row, $da);
                                        $dev = hieucon_dh_parse_device_details($row->device_info);
    ...
    function openResultModal(code, auth) {
        ...
        document.getElementById('resultIframe').src = '<?php echo esc_js(site_url('/ket-qua-dh')); ?>?code=' + code + '&auth=' + auth + '&iframe=1';
