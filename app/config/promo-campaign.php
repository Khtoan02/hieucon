...
function hieucon_promo_campaign_metabox_html($post)
{
    ...
    <table class="form-table">
        ...
        <tr>
            ...
            <td>
                <input type="number" id="promo_discount_value" name="promo_discount_value"
                    value="<?php echo esc_attr($discount_value); ?>" class="regular-text"
                    placeholder="Ví dụ: 20 (cho 20%) hoặc 50000 (cho 50.000đ)" required min="0">
            </td>
        </tr>
        <tr>
            ...
            <td>
                <select id="promo_target" name="promo_target" class="regular-text">
                    ...
                    <option value="new" <?php selected($promo_target, 'new'); ?>>Khách hàng mới (chưa mua khóa học/ebook)
                    </option>
                    <option value="loyal" <?php selected($promo_target, 'loyal'); ?>>Khách hàng thân thiết (đã sở hữu ít
                        nhất 1 khóa học/ebook)</option>
                </select>
            </td>
        </tr>
        <tr>
            ...
            <td>
                <?php if (!empty($ebooks)): ?>
                    <div
                        style="max-height: 250px; overflow-y: auto; border: 1px solid #ccd0d4; padding: 10px; background: #fff; border-radius: 4px;">
                        <?php foreach ($ebooks as $ebook): ?>
                            <label style="display: block; margin-bottom: 6px; font-weight: 500;">
                                <input type="checkbox" name="promo_applied_ebooks[]" value="<?php echo esc_attr($ebook->ID); ?>"
                                    <?php checked(in_array($ebook->ID, $applied_ebooks)); ?>>
                                <?php echo esc_html($ebook->post_title); ?>
                            </label>
                        <?php endforeach; ?>
                    </div>
                    <p class="description">Tích chọn các Cẩm nang được áp dụng chương trình khuyến mãi này.</p>
                <?php else: ?>
                    <p style="color: #9ca3af; font-style: italic;">Chưa có Cẩm nang nào được xuất bản trên hệ thống.</p>
                <?php endif; ?>
            </td>
        </tr>
    </table>
