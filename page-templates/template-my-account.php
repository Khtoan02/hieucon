...
<main id="primary" class="site-main min-h-screen py-12 md:py-20 bg-gradient-to-tr from-slate-50 via-slate-100 to-orange-50/20">
    <div class="max-w-[1000px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <!-- Header tài khoản -->
        <div class="flex flex-col md:flex-row items-center justify-between gap-6 mb-10 bg-white/70 backdrop-blur-xl border border-white p-6 rounded-[2rem] shadow-sm">
            <div class="flex items-center gap-4 text-center md:text-left flex-col md:flex-row">
                <div class="w-16 h-16 rounded-full bg-navy/5 flex items-center justify-center text-navy font-bold text-2xl border border-navy/10 uppercase">
                    <?php echo mb_substr( esc_html( $current_member->full_name ), 0, 1, 'utf-8' ); ?>
                </div>
                <div>
                    <h1 class="text-xl md:text-2xl font-serif font-bold text-navy"><?php echo esc_html( $current_member->full_name ); ?></h1>
                    <div class="flex flex-wrap items-center gap-2 mt-1.5 justify-center md:justify-start">
                        <span class="text-xs font-semibold text-navy/60"><?php echo esc_html( $current_member->email ); ?></span>
            ...
            <button type="button" onclick="handleLogout()" class="px-5 py-2.5 bg-red-50 hover:bg-red-100 text-red-600 rounded-xl font-bold text-xs border border-red-100 transition-colors flex items-center gap-2">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
            ...
            <div class="md:col-span-1 space-y-2">
                <button type="button" onclick="switchAccountTab('profile')" id="menu-profile-btn" class="w-full text-left px-5 py-3.5 rounded-2xl text-sm font-bold transition-all text-navy bg-white shadow-soft border border-white flex items-center gap-2">
                    <i data-lucide="user" class="w-4 h-4"></i> Thông tin cá nhân
                </button>
                <button type="button" onclick="switchAccountTab('password')" id="menu-password-btn" class="w-full text-left px-5 py-3.5 rounded-2xl text-sm font-bold transition-all text-navy/60 hover:text-navy hover:bg-white/50 flex items-center gap-2">
                    <i data-lucide="key" class="w-4 h-4"></i> Đổi mật khẩu
                </button>
                <button type="button" onclick="switchAccountTab('courses')" id="menu-courses-btn" class="w-full text-left px-5 py-3.5 rounded-2xl text-sm font-bold transition-all text-navy/60 hover:text-navy hover:bg-white/50 flex items-center gap-2">
            ...
            <!-- Content Area Card -->
            <div class="md:col-span-3 bg-white/70 backdrop-blur-xl border border-white shadow-elegant rounded-[2.5rem] p-6 md:p-8">
                
                <div id="account-alert" class="hidden mb-6 p-4 rounded-2xl text-xs md:text-sm font-semibold transition-all duration-300 transform scale-95 opacity-0"></div>

                <!-- ================= TAB 1: THÔNG TIN CÁ NHÂN ================= -->
                <div id="tab-profile-view" class="transition-opacity duration-300">
                    <h2 class="text-lg font-serif font-bold text-navy mb-6 pb-2 border-b border-slate-100">Cập nhật Thông tin Cá nhân</h2>
                    ...
                    <form id="form-profile-update" onsubmit="triggerProfileUpdateOTP(event)" class="space-y-5">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            <div class="space-y-1">
                                ...
                                <input type="text" name="full_name" required value="<?php echo esc_attr( $current_member->full_name ); ?>" class="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white/50 focus:bg-white focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-sm">
                            </div>
                            <div class="space-y-1">
                                <label class="block text-xs font-bold text-navy/70 uppercase tracking-widest pl-1">Địa chỉ Email</label>
                                <input type="email" value="<?php echo esc_attr( $current_member->email ); ?>" readonly class="w-full px-4 py-3 rounded-xl border border-slate-100 bg-slate-50 text-navy/40 text-sm font-semibold cursor-not-allowed select-none">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            <div class="space-y-1">
                                ...
                                <input type="tel" name="phone_number" value="<?php echo esc_attr( $current_member->phone_number ); ?>" class="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white/50 focus:bg-white focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-sm">
                            </div>
                            <div class="space-y-1">
                                <label class="block text-xs font-bold text-navy/70 uppercase tracking-widest pl-1">Ngày sinh</label>
                                <input type="date" name="date_of_birth" value="<?php echo esc_attr( $current_member->date_of_birth ); ?>" class="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white/50 focus:bg-white focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-sm">
                            </div>
                        </div>

                        <!-- Turnstile Captcha hiển thị trước khi lưu -->
                        <?php if ( ! empty( $turnstile_sitekey ) ) : ?>
                            <div class="cf-turnstile" data-sitekey="<?php echo esc_attr( $turnstile_sitekey ); ?>" data-theme="light"></div>
                        <?php endif; ?>

                        <div class="pt-2">
                            <button type="submit" class="px-6 py-3.5 bg-navy hover:bg-navy/90 text-white rounded-xl font-bold text-xs shadow-md transition-colors flex items-center gap-2">
                ...
                <div id="tab-password-view" class="hidden transition-opacity duration-300">
                    <h2 class="text-lg font-serif font-bold text-navy mb-6 pb-2 border-b border-slate-100">Thay đổi Mật khẩu Bảo mật</h2>
                    
                    <form id="form-password-update" onsubmit="triggerPasswordUpdateOTP(event)" class="space-y-5">
                        <div class="space-y-1">
                            <label class="block text-xs font-bold text-navy/70 uppercase tracking-widest pl-1">Mật khẩu mới</label>
                            <input type="password" id="new-password" name="password" required class="w-full px-4 py-3.5 rounded-xl border border-slate-200 bg-white/50 focus:bg-white focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-sm" placeholder="Tối thiểu 8 ký tự">
                        </div>

                        <div class="space-y-1">
                            <label class="block text-xs font-bold text-navy/70 uppercase tracking-widest pl-1">Xác nhận mật khẩu mới</label>
                            <input type="password" id="confirm-password" required class="w-full px-4 py-3.5 rounded-xl border border-slate-200 bg-white/50 focus:bg-white focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-sm" placeholder="Nhập lại mật khẩu mới">
                        </div>
                ...
                <div id="tab-courses-view" class="hidden transition-opacity duration-300">
                    <h2 class="text-lg font-serif font-bold text-navy mb-6 pb-2 border-b border-slate-100">Khóa học của tôi</h2>
                    ...
                    if ( empty( $enrolled_ids ) && ! $is_privileged ) {
                        ...
                        <div class="text-center py-12">
                            ...
                            <p class="text-slate-550 font-medium mb-6 text-sm">Bạn chưa sở hữu khóa học nào trên hệ thống.</p>
                            <a href="<?php echo esc_url( home_url( '/courses/' ) ); ?>" class="inline-flex items-center gap-2 px-6 py-3.5 bg-navy hover:bg-navy/90 text-white rounded-xl font-bold text-xs shadow-md transition-colors">
                        ...
                        if ( $my_courses_query->have_posts() ) {
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                <?php while ( $my_courses_query->have_posts() ) : $my_courses_query->the_post(); 
                                ...
                                ?>
                                    <div class="bg-white/50 border border-slate-100 rounded-3xl p-4 flex flex-col justify-between group hover:bg-white hover:shadow-soft transition-all duration-300">
                                        <div>
                                            ...
                                            <div class="relative aspect-video w-full rounded-2xl overflow-hidden bg-slate-150 mb-4">
                                                ...
                                                <div class="absolute top-3 left-3">
                                                    <span class="px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider border <?php echo $level_class; ?>">
                                                        <?php echo esc_html( $level_label ); ?>
                                                    </span>
                                                </div>
                                            </div>
                                            
                                            <h3 class="font-serif font-bold text-navy text-sm md:text-base group-hover:text-primary transition-colors line-clamp-2 mb-2">
                                                <a href="<?php the_permalink(); ?>"><?php the_title(); ?></a>
                                            </h3>
                                            
                                            <div class="flex items-center gap-4 text-[10px] md:text-[11px] text-slate-550 font-semibold mb-4">
                                        ...
                                        <a href="<?php echo esc_url( $first_lesson_url ); ?>" class="w-full py-3 bg-emerald-50 hover:bg-emerald-600 hover:text-white text-emerald-600 rounded-xl font-bold text-xs transition-colors flex items-center justify-center gap-1.5 border border-emerald-100">
                ...
                <div id="tab-redeem-view" class="hidden transition-opacity duration-300">
                    <h2 class="text-lg font-serif font-bold text-navy mb-6 pb-2 border-b border-slate-100">Kích hoạt Khóa học của Bạn</h2>
                    
                    <div class="bg-gradient-to-r from-orange-50/50 via-amber-50/30 to-slate-50 p-6 rounded-3xl border border-orange-100/50 mb-6 flex items-start gap-4">
                        <div class="w-10 h-10 rounded-full bg-orange-100 text-orange-600 flex items-center justify-center shrink-0 border border-orange-200">
                            <i data-lucide="gift" class="w-5 h-5"></i>
                        </div>
                        <div>
                            <h4 class="text-sm font-bold text-navy mb-1">Mã kích hoạt E-Learning Hieucon</h4>
                            <p class="text-xs text-slate-500 leading-relaxed font-medium">Nhập mã kích hoạt của bạn để nhận toàn quyền truy cập bài giảng và bài tập thực hành. Mỗi mã chỉ được sử dụng một lần cho một tài khoản hội viên.</p>
                        </div>
                    </div>

                    <form id="form-redeem-code" onsubmit="handleRedeemCode(event)" class="space-y-5">
                        <div class="space-y-1">
                            <label class="block text-xs font-bold text-navy/70 uppercase tracking-widest pl-1">Nhập mã kích hoạt (Redeem Code)</label>
                            <input type="text" id="redeem-code-input" required class="w-full px-4 py-3.5 rounded-xl border border-slate-200 bg-white/50 focus:bg-white focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-sm font-mono tracking-wider placeholder:font-sans placeholder:tracking-normal" placeholder="Ví dụ: HIEUCON-XXXX-XXXX">
                        </div>

                        <div class="pt-2">
                            <button type="submit" id="btn-redeem-submit" class="px-6 py-3.5 bg-navy hover:bg-navy/90 text-white rounded-xl font-bold text-xs shadow-md transition-all duration-300 flex items-center gap-2">
...
<div id="otp-modal" class="fixed inset-0 z-50 flex items-center justify-center hidden">
    ...
    <!-- Hộp thoại Modal -->
    <div class="bg-white/95 backdrop-blur-xl border border-white max-w-[450px] w-full mx-4 rounded-3xl p-6 md:p-8 relative z-10 shadow-2xl animate-fade-in">
        
        <button type="button" onclick="closeOTPModal()" class="absolute top-4 right-4 text-navy/40 hover:text-navy transition-colors">
            <i data-lucide="x" class="w-5 h-5"></i>
        </button>

        <div class="text-center">
            <div class="w-12 h-12 bg-orange-50 text-secondary rounded-full flex items-center justify-center mx-auto mb-4 border border-orange-100">
                <i data-lucide="mail-check" class="w-6 h-6"></i>
            </div>
            
            <h3 class="text-lg font-serif font-bold text-navy">Xác thực OTP Bảo mật</h3>
            <p class="text-navy/60 text-xs md:text-sm mt-2 font-medium">Để bảo vệ thông tin tài khoản, chúng tôi đã gửi 1 mã xác thực OTP gồm 6 chữ số về Email của bạn. Hãy kiểm tra hộp thư.</p>
        </div>
        ...
        <form id="form-otp-verification" onsubmit="submitOTPVerifiedData(event)" class="mt-6 space-y-4">
            <div class="space-y-1">
                <label class="block text-xs font-bold text-navy/70 text-center uppercase tracking-widest">Mã OTP (6 chữ số)</label>
                <input type="text" id="modal-otp-input" maxlength="6" required class="w-full px-4 py-3.5 rounded-xl border border-slate-200 text-center tracking-[1em] text-lg font-bold focus:ring-2 focus:ring-primary/20 focus:border-primary bg-slate-50 focus:bg-white transition-all" placeholder="000000">
            </div>

            <!-- Turnstile xác thực riêng trong Modal để đảm bảo tính an toàn chống bypass -->
            <?php if ( ! empty( $turnstile_sitekey ) ) : ?>
                <div class="cf-turnstile flex justify-center" id="modal-turnstile-container" data-sitekey="<?php echo esc_attr( $turnstile_sitekey ); ?>" data-theme="light"></div>
            <?php endif; ?>

            <button type="submit" id="btn-modal-confirm" class="w-full py-3.5 bg-navy hover:bg-navy/90 text-white rounded-xl font-bold text-xs shadow-md transition-colors flex items-center justify-center gap-2">
